...
class ToRGB(nn.Module):
    ...
    def flops(self):
        ...
        if self.is_upsample:
            ...
            # N x Cout x H x W x  (Cin x Kw x Kh + bias)
            flops = 1 * 3 * (2 * self.resolution + 3) * \
                (2 * self.resolution + 3) * (3 * kernel_ops)
        return flops
...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, rope_theta=10.0, qk_scale=None, attn_drop=0., proj_drop=0., pretrained_window_size=[0, 0]):
        ...
        t_x, t_y = init_t_xy(
            end_x=self.window_size[1], end_y=self.window_size[0])
...
class Generator(nn.Module):
    ...
    def forward(
        ...
        # L = numbers of pathes, and we want it to resize to (B, C, size, size) to make it to Bs images
        # B = numbers of images
        # C = Channels
        B, L, C = x.shape
